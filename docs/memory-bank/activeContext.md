...
- [2026-06-04] **Extraction de l'action GitHub → dépôt dédié `trivoallan/regis-action`** (branche `claude/github-action-extraction-6zdu5`, breaking):
  ...
  - **Caveat périmètre MCP** : le dépôt `regis-action` a été créé via l'API, mais le périmètre de session (`trivoallan/regis` seul) a bloqué le push des fichiers. Contenu du nouveau dépôt mis en attente sous `regis-action-staging/` pour un push manuel / élargissement de périmètre. `feat(ci)!` → bump pré-v1.
