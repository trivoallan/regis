...
* **Skopeo**: Extracts multi-architecture metadata, labels, and layer information. (xref:schemas/skopeo.adoc[Schema])
* **Trivy**: Generates Software Bill of Materials (SBOM) and performs vulnerability scanning. (xref:schemas/trivy.adoc[Schema])
* **Hadolint**: Lints Dockerfiles for best practices. (xref:schemas/hadolint.adoc[Schema])
* **Dockle**: Container image linter for security and best practices. (xref:schemas/dockle.adoc[Schema])
* **Versioning**: Validates semantic versioning consistency. (xref:schemas/versioning.adoc[Schema])
* **Freshness**: Calculates image age. (xref:schemas/freshness.adoc[Schema])
...
It uses `Jinja2` templates to produce rich, themeable HTML reports for human review and structured JSON (see xref:schemas/report.adoc[Report Schema]) for automated processing.
