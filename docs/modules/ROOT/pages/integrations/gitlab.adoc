...
[TIP]
====
To quickly bootstrap a new GitLab repository pre-configured with `regis-cli` and GitLab CI, you can use our xref:../commands.adoc#bootstrap[Project Bootstrapping] command.
====
...
For more details on configuring checklist items, see the xref:playbooks.adoc#_mr_description_checklists[Playbooks Reference].
